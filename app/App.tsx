...
export default function App() {
    ...
    const loop = async () => {
      ...
      if (predictions.length > 0) {
        for (let i = 0; i < predictions.length; i++) {
          const start = predictions[i].topLeft;   //start[0]=x start[1]=y
          const end = predictions[i].bottomRight;  //end[0]=x end[1]=y
          ...
          tf.tidy(() => {// 함수 종료 시 slicedTensor는 자동으로 메모리에서 해제됩니다.
            //얼굴영역만 자르기
            const faceTensor = tf.slice(imageTensor, [Math.round(start[1]), Math.round(start[0]), 0], [Math.round(size[1]), Math.round(size[0]), 3]);
            ...
            meanValue.data().then(data => {
              ...
              console.log("평균시계열데이터:"+gChannelMeans);
            })
          });
        }
      }
  ...
  const getTextureRotationAngleInDegrees = async (cameraType) => {
